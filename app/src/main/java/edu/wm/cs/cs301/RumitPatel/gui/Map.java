...
public class Map {
		...
		for (int y = minY; y <= maxY; y++)
			for (int x = minX; x <= maxX; x++) {
				...
				if (x < mazeWidth)
					...
					drawHorizontalLine(showMaze, x, y, startX, startY);
				if (y < mazeHeight)
					//drawVerticalLine(g, showMaze, x, y, startX, startY);
					drawVerticalLine(showMaze, x, y, startX, startY);
			}
	 ...
	 */
	private void drawVerticalLine( boolean showMaze, int x, int y, int startX, int startY) {
		if (hasAVerticalWall(x, y) && 
				(seenWalls.hasWall(x, y, CardinalDirection.West) || showMaze)) {
			mp.setColor(seenWalls.hasWall(x, y, CardinalDirection.West) ? -1 : -8355712);
	...
	private void drawHorizontalLine( boolean showMaze, int x, int y, int startX,
		...
		if (hasAHorizontalWall(x, y) && (seenWalls.hasWall(x,y, CardinalDirection.North) || showMaze) ) {
			mp.setColor(seenWalls.hasWall(x,y, CardinalDirection.North) ? -1 : -8355712);
